...
function omega.scripts.onherodeath()
	if Game.GetGameDifficulty() >= Game.Difficulty.Legendary then
		local soulRandomizer = 0
		local player = Game.GetLocalPlayer();
		-- Hero soul drop rate at 90%
		--math.randomseed(Time.Now())
		--soulRandomizer = math.random(1,100)
		--if soulRandomizer >= 11 then
...
function omega.scripts.clonechestonOpen(objectId)
	...
	chestEntity:Destroy()
	local newCreature = Entity.Create('records/omega/creatures/enemies/boss&quest/om_boss_challenge20.dbr');
...
local tcreature={'records/omega/creatures/enemies/boss&quest/om_boss_challenge20.dbr'}
